...
void random_raspberry(int font_rotation, int raspberry_rotation);
void loop() {
    static int raspberry_rotation = 0;
    static int font_rotation = 0;
    for (int j = 0; j < 8; j++) {
        setup();
        for (int i = 0; i < 180; i++) {
            random_raspberry(font_rotation, raspberry_rotation);
        }
        font_rotation++;
        if (font_rotation >= 4) font_rotation = 0;
    }
    raspberry_rotation++;
    if (raspberry_rotation >= 4) raspberry_rotation = 0;
}
...
void random_raspberry(int font_rotation, int raspberry_rotation) {
    ...
    int temp1, temp2;
    display.setRotation(raspberry_rotation);
    display.fillScreen(SSD1306_BLACK);
    for (int i = 0; i < RASPBERRY_N; i++ ) {
    ...
    display.setRotation(font_rotation);
